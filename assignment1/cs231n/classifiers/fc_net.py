    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.normalization == "batchnorm":
                ...
                out, cache_norm = batchnorm_forward(out, gamma, beta, self.bn_params[i - 1])
            elif self.normalization == "layernorm":
                gamma, beta = self.params[f"gamma{i}"], self.params[f"beta{i}"]
                out, cache_norm = layernorm_forward(out, gamma, beta, self.bn_params[i - 1])
            ...
            self.caches.append((cache_affine, cache_norm, cache_relu, cache_drop))
            
        W, b = self.params[f"W{self.num_layers}"], self.params[f"b{self.num_layers}"]
        scores, cache_last = affine_forward(out, W, b)
        self.caches.append(cache_last)
        ...
        cache_last = self.caches.pop()
        dout, dW, db = affine_backward(dscores, cache_last)
        grads[f"W{self.num_layers}"] = dW + self.reg * self.params[f"W{self.num_layers}"]
        grads[f"b{self.num_layers}"] = db
        
        for i in range(self.num_layers - 1, 0, -1):
            cache_affine, cache_norm, cache_relu, cache_drop = self.caches.pop()
