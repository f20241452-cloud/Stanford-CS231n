...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1.0 / N) / std * (N * dx_normalized - np.sum(dx_normalized, axis=0) - x_normalized * np.sum(dx_normalized * x_normalized, axis=0))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_backward(dout, cache):
    ...
    dx = (1.0 / D) / std * (D * dx_normalized - np.sum(dx_normalized, axis=1, keepdims=True) - x_normalized * np.sum(dx_normalized * x_normalized, axis=1, keepdims=True))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dgamma, dbeta
        ...
        for f in range(F):
            for h in range(H_out):
                for w_idx in range(W_out):
                    ...
                    w_end = w_start + WW
                    out[n, f, h, w_idx] = np.sum(x_padded[n, :, h_start:h_end, w_start:w_end] * w[f, :, :, :])
        ...
        for f in range(F):
            for h in range(H_out):
                for w_idx in range(W_out):
                    ...
                    dw[f, :, :, :] += x_padded[n, :, h_start:h_end, w_start:w_end] * dout[n, f, h, w_idx]
                    dx_padded[n, :, h_start:h_end, w_start:w_end] += w[f, :, :, :] * dout[n, f, h, w_idx]
        ...
        for c in range(C):
            for h in range(H_out):
                for w_idx in range(W_out):
                    ...
                    out[n, c, h, w_idx] = np.max(x[n, c, h_start:h_end, w_start:w_end])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
        ...
        for c in range(C):
            for h in range(H_out):
                for w_idx in range(W_out):
                    ...
                    dx[n, c, h_start:h_end, w_start:w_end] += mask * dout[n, c, h, w_idx]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    sample_mean = np.mean(x_grouped, axis=(2, 3, 4), keepdims=True)
    sample_var = np.var(x_grouped, axis=(2, 3, 4), keepdims=True)
    x_normalized = (x_grouped - sample_mean) / np.sqrt(sample_var + eps)
    x_normalized = x_normalized.reshape(N, C, H, W)
    out = gamma * x_normalized + beta
    cache = (x, sample_mean, sample_var, x_normalized, gamma, beta, eps, G)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # This will be extremely similar to the layer norm implementation.        #
    ###########################################################################
    x, sample_mean, sample_var, x_normalized, gamma, beta, eps, G = cache
    N, C, H, W = x.shape
    D = C // G * H * W
    ...
    x_centered = x_grouped - sample_mean
    std = np.sqrt(sample_var + eps)
    
    dx_grouped = (1.0 / D) / std * (D * dx_normalized_grouped - np.sum(dx_normalized_grouped, axis=(2, 3, 4), keepdims=True) - 
                 x_centered * np.sum(dx_normalized_grouped * x_centered, axis=(2, 3, 4), keepdims=True))
